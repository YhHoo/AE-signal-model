...
dataset, label = data.leak_1bar_in_cwt_xcor_maxpoints_vector(dataset_name=2,
                                                             f_range_to_keep=(50, 100),
                                                             class_to_keep=[0, 3, 7, 10],
                                                             shuffle=False)
...
#
# # label binarizer cn even work for jumping labels, e.g. [0, 0, 2, 4, 6, 8]
# lb_encoder = LabelBinarizer()
# train_y_cat = lb_encoder.fit_transform(train_y)
# test_y_cat = lb_encoder.fit_transform(test_y)
# model = fc_leak_1bar_max_vec_v2(input_shape=nn_input_shape, output_neuron=4)
# model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc'])
# model_logger = ModelLogger(model, model_name='fc_leak_1bar_max_vec_v2')
# history = model.fit(x=train_x,
#                     y=train_y_cat,
...
def create_model(optimizer='adam'):
    model = fc_leak_1bar_max_vec_v2(input_shape=nn_input_shape, output_neuron=num_classes)
    # optimizer
